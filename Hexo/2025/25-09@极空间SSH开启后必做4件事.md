...
- **隐私安全**：宝塔面板使用时强制~~要求注册宝塔账户~~^[如需使用其应用商店，依旧需要注册登录账户（仅支持手机号注册）。]并上传服务器配置信息，此举存在潜在的隐私风险。
...
在安装 1Panel 的过程中，若选择配置镜像加速，则会覆盖 Docker 的 `daemon.json` 文件，并自动将原始内容备份到同目录下的 `daemon.json.1panel_bak` 文件。此操作将导致原先已安装的容器丢失，为解决该问题，可以将备份文件中的内容筛选恢复并重启 Docker 引擎：
...
// 图片地址变量
$background-image: 'https://static.szyink.com/storage/wallhaven/wallhaven-3qw16y.jpg';

// 颜色变量 (RGBA 格式)
$colors: (
  // 登录框
  login-bg: rgba(20, 20, 25, 0.75),
  ...
  // 按钮
  btn-bg: rgba(70, 130, 180, 0.8),
  btn-bg-hover: rgba(100, 149, 237, 0.9),
  btn-text: rgba(255, 255, 255, 0.95),

  // 复选框
  checkbox-bg: rgba(255, 255, 255, 0.15),
  checkbox-border: rgba(255, 255, 255, 0.3),
  checkbox-checked: rgba(70, 130, 180, 0.9),

  // 输入框
  input-bg: rgba(255, 255, 255, 0.08),
  input-border: rgba(255, 255, 255, 0.2),
  input-border-focus: rgba(100, 149, 237, 0.6),

  // 装饰线
  underline: rgba(100, 149, 237, 0.8),
);
...
@mixin glass-effect {
  ...
  border: 1px solid map-get($colors, login-border);
  box-shadow: 0 8px 32px map-get($colors, login-shadow);
}
  ...
  #app {
    ...
    .contain:has(.login) {
      ...
      .login {
        ...
        .titleText {
          font-weight: 600;
          background: linear-gradient(to bottom, transparent 60%, rgb(243, 226, 228) 0) no-repeat;
        ...
        .el-button {
          width: 100%;
          background: rgba(0 , 149, 237, 0.35) !important;
          ...
          &:hover {
            background: map-get($colors, btn-bg-hover) !important;
        ...
        .qrcode-code::after {
          ...
          line-height: 20px;
          display: block;
          content: 'Please sit back and relax\A请坐和放宽';
...
![登录页](https://cdn.jsdelivr.net/gh/inkss/inkss-cdn@main/img/article/25-09@极空间SSH开启后必做4件事/登录页.png)

![桌面页](https://cdn.jsdelivr.net/gh/inkss/inkss-cdn@main/img/article/25-09@极空间SSH开启后必做4件事/桌面页.png)

![极影视](https://cdn.jsdelivr.net/gh/inkss/inkss-cdn@main/img/article/25-09@极空间SSH开启后必做4件事/极影视.png)

![应用页](https://cdn.jsdelivr.net/gh/inkss/inkss-cdn@main/img/article/25-09@极空间SSH开启后必做4件事/应用页.png)
...
declare -A CONFIG=(
    ...
    [favicon_link]="<link rel=\"icon\" href=\"/home/favicon.ico\">"  # 新增：可选 favicon 注入
)

backup_path="${CONFIG[file_path]}.bak"
tmp_file="${CONFIG[file_path]}.tmp"
tmp_file2="${CONFIG[file_path]}.tmp2"
...
check_prerequisites() {
    ...
    local target_dir="${CONFIG[target_dir]}"
    ...
    log "INFO" "检查通过：文件和目录存在且可写。"
}

# 备份文件 (检查是否需要更新备份)
backup_file() {
    local file_path="${CONFIG[file_path]}"
    if [ -f "$backup_path" ] && cmp -s "$file_path" "$backup_path"; then
        log "INFO" "备份文件 $backup_path 已存在且当前，无需更新。"
        return 0
    fi
...
# 替换内容 (优化：仅在需要时创建 temp 文件；添加验证)
replace_content() {
    local file_path="${CONFIG[file_path]}"
    local title_search="${CONFIG[title_search]}"
    local title_replace="${CONFIG[title_replace]}"
    local favicon_link="${CONFIG[favicon_link]}"

    # 检测标题是否需要替换
    if ! grep -q "$title_search" "$file_path"; then
    ...
    # 仅在需要时创建 temp
    cp "$file_path" "$tmp_file" || { log "ERROR" "创建临时文件失败。"; exit 1; }

    # 替换标题
    if ! sed "s|$title_search|$title_replace|" "$tmp_file" > "$tmp_file2"; then
        rm -f "$tmp_file2"
    ...
    # 可选：注入 favicon 链接 (如果不存在)
    if ! grep -q 'rel="icon"' "$tmp_file"; then
        sed "s|</head>|$favicon_link</head>|" "$tmp_file" > "$tmp_file2" || { log "ERROR" "Favicon 注入失败。"; exit 1; }
        mv "$tmp_file2" "$tmp_file"
        log "INFO" "已注入 favicon 链接。"
    fi

    # 更新文件并验证
    mv "$tmp_file" "$file_path" || { log "ERROR" "更新文件失败。"; exit 1; }
    if grep -q "${CONFIG[title_replace]#*<title>*</title>}" "$file_path"; then  # 检查新标题存在
        log "INFO" "替换成功：已更新标题。"
    else
        log "ERROR" "替换后验证失败，回滚到备份。"
...
copy_custom_files() {
    local target_dir="${CONFIG[target_dir]}"
    local sources=("${CONFIG[css_source]}" "${CONFIG[favicon_source]}")
    local copied=0
    ...
    for src in "${sources[@]}"; do
        ...
        if cmp -s "$src" "$dest"; then
            ((copied++))
            log "INFO" "已更新 $(basename "$src")。"
        fi
    done

    if [ "$copied" -eq ${#sources[@]} ]; then
        log "INFO" "复制成功：所有自定义文件已更新。"
    fi
}
...
{% folding cyan, check_sshdconfig.sh %}
