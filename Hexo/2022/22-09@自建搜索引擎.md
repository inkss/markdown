...
{% note warning::注：hexo-uuid 实际上是使用 `uuid.v1()` 根据时间戳在 `before_renderPost` 阶段时生成，也就意味若使用自动化部署时，起码在本地执行一次 `hexo server`，保证记录值唯一。<br>对于 MeiliSearch，相同的主键在上传时视为更新，不同时新增。 %}
