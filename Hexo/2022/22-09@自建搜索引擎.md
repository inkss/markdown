...
本文记录 MeiliSearch 的搭建及如何在 Hexo 博客上使用，原则上来说，MeiliSearch 在前端搜索上使用了公共开源的 `instantsearch.js` 项目，这个项目是由 Aligolia 维护，所以在调用方面与 Aligolia 大差不差，部署起来问题不大。
...
> 仔细查看的话会发现它是在判断操作系统详情，然后去 Github Releases 下载对应的发行包，由于众所周知的原因，境内服务器访问 `raw.github.com` 可能不那么利索，这里你可以手动下载你服务器所需发行包后自行上传至相关目录，最后将程序名称更改为 *meilisearch* or *meilisearch.exe*。
...
先解决主键，事实上 Hexo 有个唯一 ID： `_id`，但是它记录在博客目录下的 `db.json` 文件中，也就是执行 `hexo clean` 时就被重现生成凉凉了。秉着有现成就绝不自己写能偷懒就偷懒的态度，找到了 [chekun/hexo-uuid](https://github.com/chekun/hexo-uuid) 这个插件，只需要安装它就能够在 `post/page` 页面中自动生成 `uuid` 值。
...
在本地搜索时，用到了 [hexo-generator-json-content](https://github.com/alexbruno/hexo-generator-json-content)，按照其说明，改吧改吧配置可以直接拿过来给 MeiSearch 使用，计划通。
...
MeiliSearch 可以利用 [Instant Meilisearch](https://github.com/meilisearch/instant-meilisearch) 搭配 instantsearch.js 使用，能达到和 Aligolia 极其相似的调用过程，迁移起来也非常方便。

*此部分内容有时间再补充吧~ {% emoji tiaopi %}*
...
适配时最尝试过 [docs-searchbar.js](https://github.com/meilisearch/docs-searchbar.js) 这个项目，他需要先利用 [docs-scraper](https://github.com/meilisearch/docs-scraper) 抓取网站接口，然后在生成文档搜索。使用起来一言难尽，配置项特别少，很不灵活。
