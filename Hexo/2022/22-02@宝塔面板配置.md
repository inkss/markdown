...
① 搜索 `recommend_paid_version` 函数，清空函数体；② 继续搜索 ***// 推荐安装软件***，清空 try 块。
...
① 去掉傻 logo：搜索 `class="login-images"`，删除对应内容。

② 去掉扫码登录：搜索 `class="entrance"`，删除对应内容；

③ 去掉变量调用：搜索 ***切换二维码***，修改成 `var entrance = {}`。

④ 去掉忘记密码：搜索 `class="forget_pwd"`，删除对应内容。
