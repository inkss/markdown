...
为了便于后期管理及访问，此处建议绑定静态地址，输入项从下往下以此为：子网、静态IP地址、网关、DNS解析地址。此处根据个人实际情况填写。
...
此处使用默认的 LVM 格式的引导分区，便于以后动态扩增。但需注意，默认没有把所有空间都分配给根目录。见下图，50G 分区中根目录只使用了 23.996G 空间，此处主动修改。
