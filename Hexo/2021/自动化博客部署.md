...
但好巧不巧我选择了最复杂的路线，由于我自建了 Gitea 代码托管，秉承着不能浪费的原则：不如将一份博客源码同步到 Gitea 仓库 {% emoji huaixiao %}，然后顺带还可以同步进 Github 仓库，这里用到的工具是 [hexo-deployer-git](https://github.com/hexojs/hexo-deployer-git)，基本的 Action 命令可以这样写：
...
除此之外，还需要手动修改配置文件，在配置文件中添加以下内容：

{% folding cyan, Supervisord 子配置文件 %}
```yml 在最后一行添加如下内容
