...
updated: '2021-08-04 19:42'
abbrlink: 42987b6b
hideTitle: true
headimg: ../../img/article/自动化博客部署/main-new.gif
background: ../../img/article/自动化博客部署/wallhaven-48o37j.png
description: 说是自动化博客部署，其实是记录了下枋柚梓的猫会发光的部署过程，基本概括来说就是通过 Github Action 处理代码的提交，部署博客到 Github、Gitea、OSS 中，此外还包含一些 GPG 的签名提交、自托管主机遇到的一些坑和一些其它功能的使用过程。
references:
...
<div class="img-wrap">
  <div class="img-bg" style="background:var(--color-white-png)">
    <img class="img" src="../../img/article/自动化博客部署/Hexo博客部署架构.svg" alt="Hexo博客部署架构" style="max-height: 800px;">
  </div>
...
如你所见，这个部署流程是繁杂的，如果只是将 Hexo 博客部署到 Github、OSS、CVM 等，其实都有对应现成的轮子可以用。不过出于个人定制的缘故，我将整个流程拆分成了三部分：整站内容部署到服务器、Github 仓库；静态文件（js,css,fonts）单独同步到另一个 Github 仓库，并且使用 Jsdelivr 加速访问静态库；而图片类文件则同步到腾讯云对象存储，以方便使用它的图片处理功能（自适应 Webp 自适应、Guetzli 自适应等）。
...
如你所见，我也只是做了这些而已嘛 {% emoji mianwubiaoqing %}，下面记录上述过程中的踩坑历程。
...
![流程](../../img/article/自动化博客部署/image-20210728163902653.png)

我将部署流程拆分成了三部分：部署到 Git、部署到 OSS、同步文件到 Git，它们用到的工具分别是 `hexo-deployer-git` 、 `hexo-deployer-cos-cdn` 以及简单的 Bash 命令。
...
部署到对象存储就和我的域名没有关系了，此部分可以使用 Github 的主机，由于将部署到 CVM 和 OSS 拆分成两个流程，那么配置文件自然要独立成两个，我在 Hexo 文档中看到关于多个配置文件整个使用的：
...
把 Github Action 部署到自托管服务器后，面临一个无奈的现实：在 Github 仓库克隆的过程中有一定概率连不上，服务器不太敢部署上小飞机，所以调整了下部署流程，主要是保证博客正常的部署。

![](../../img/article/自动化博客部署/image-20210804193933314.png)

## 五、文末总结

其实整的那么复杂，主要是为了折腾，最起码还是蛮有趣的 {% emoji ku %}。
